...
class RestAPI:
    def __init__(self, config, miner, stats):
        ...
        # clock_manager exists only after ASIC init; /influx/stats must work without it (LM75-only mode).
        self.cm = getattr(self.asics, "clock_manager", None)
        self.config = config
        self.stats = stats
    ...
    def get_clocks(self):
        if self.cm is None:
            return jsonify({"error": "ASIC not initialized"}), 503
    ...
    def get_influx_stats(self):
        with self.stats.lock:
            stats = {
                ...
                "asic_frequency_mhz": self.stats.asic_frequency_mhz,
                "asic_frequency_target_mhz": self.stats.asic_frequency_target_mhz,
