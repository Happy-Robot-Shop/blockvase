    ...
    def count_asic_chips(self):
        ...
        while True:
            ...
            if self.chip_id_response not in binascii.hexlify(data).decode('utf8'):
                if responses_seen >= 64:
                    logging.warning("Stopping chip count after %d non-chip responses", responses_seen)
                    break
                continue
        ...
        self.send(TYPE_CMD | GROUP_ALL | CMD_INACTIVE, [0x00, 0x00])
        logging.info("BM1366 chip scan: %d chip id response(s), %d total response(s)", chip_counter, responses_seen)
