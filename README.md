...
| `datum-gateway.service` / `blockvase-miner.service` | Solo path: PiAxe → local Stratum → DATUM → Knots GBT (not a public pool). Miner on by default; DATUM/hashing after setup payout address + synced node |
